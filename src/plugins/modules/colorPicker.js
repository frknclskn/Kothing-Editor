  ...
  createColorList: function (core, makeColor) {
    ...
    for (let i = 0, len = colorList.length, color; i < len; i++) {
      ...
      if (colorArr.length > 0) {
        list +=
          '<div class="ke-selector-color">' + makeColor(colorArr) + "</div>";
        colorArr = [];
      }
    ...
    list +=
      "" +
      '<form class="ke-submenu-form-group">' +
      '<input type="text" maxlength="9" class="_ke_color_picker_input ke-color-input"/>' +
      '<button type="submit" class="ke-btn-primary _ke_color_picker_submit" title="' +
      lang.dialogBox.submitButton +
      '">' +
      core.icons.checked +
      "</button>" +
      '<button type="button" class="ke-btn _ke_color_picker_remove" title="' +
