...
interface Core {
  ...
  setControllerPosition(
    ...
    referEl: Element,
    position: "top" | "bottom",
    addOffset: { left: number; top: number }
  ): void;
  ...
  insertNode(
    ...
    afterNode?: Node,
    checkCharCount?: boolean
  ): { startOffset: Node; endOffset: number } | Node | null;
   ...
   */
  detachList(
    selectedFormats: Element[],
    remove: boolean
  ): { sc: Element; ec: Element };
  ...
  actionCall(
    command: string,
    display: "command" | "submenu" | "dialog" | "container",
    target: Element
  ): void;
...
type imageInputInformation = {
  linkValue: string;
  linkNewWindow: Window;
  inputWidth: number;
  inputHeight: number;
  align: string;
  isUpdate: boolean;
...
export default class KothingEditor {
  constructor(
    context: Context,
    pluginCallButtons: Record<string, Element>,
    plugins: Record<string, Plugin>,
   ...
   */
  setOptions(options: KothingEditorOptions): void;
