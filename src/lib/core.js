...
export default function (
  ...
  const core = {
    ...
    setControllerPosition: function (controller, referEl, position, addOffset) {
      ...
      const l =
        offset.left - context.element.wysiwygFrame.scrollLeft + addOffset.left;
      const controllerW = controller.offsetWidth;
      const referElW = referEl.offsetWidth;

      const allow = util.hasClass(controller.firstElementChild, "ke-arrow")
      ...
      if (options.rtl) {
        ...
        if (rtlW > 0) {
          if (allow)
            allow.style.left =
              (controllerW - 14 < 10 + rtlW ? controllerW - 14 : 10 + rtlW) +
              "px";
        }

        const overSize =
          context.element.wysiwygFrame.offsetLeft - controller.offsetLeft;
        if (overSize > 0) {
    ...
    execCommand: function (command, showDefaultUI, value) {
      this._wd.execCommand(
        command,
        showDefaultUI,
        command === "formatBlock" ? "<" + value + ">" : value
      );
      // history stack
    ...
    resetRangeToTextNode: function () {
      ...
      if (
        ...
        (tempCon.nodeType === 1 && tempCon.childNodes.length > 0)
      ) {
            ...
            tempCon =
              ...
              !/FIGURE/i.test(tempChild[0].nodeName)
                ...
                : tempCon.previousElementSibling ||
                  ...
                  startCon;
            tempOffset =
              tempOffset > 0 ? tempCon.textContent.length : tempOffset;
          }
    ...
    appendFormatTag: function (element, formatNode) {
      ...
      if (
        (formatNode && typeof formatNode !== "string") ||
    ...
    insertComponent: function (
      ...
      if (util.isListCell(formatEl)) {
        ...
        if (
          ...
          (r.container.nodeType === 3 || util.isBreak(r.container))
        ) {
    ...
    setComponentLineBreaker: function (element) {
      ...
      if (
        ...
        b_style.left =
          util.getOffset(target).left + target.offsetWidth - w - 24 + "px";
    ...
    insertNode: function (oNode, afterNode, checkCharCount) {
      ...
      const freeFormat = util.getFreeFormatElement(
        this.getSelectionNode(),
        null
      );
      const isFormats =
      ...
      if (
        ...
        if (r.container.nodeType === 3 || util.isBreak(r.container)) {
          const depthFormat = util.getParentElement(
            ...
            function (current) {
              return (
                this.isRangeFormatElement(current) || this.isListCell(current)
              );
            }.bind(util)
          );
      ...
      const formatRange =
        range.startContainer === commonCon && util.isFormatElement(commonCon);
      const startCon = formatRange
        ? commonCon.childNodes[startOff] || commonCon.childNodes[0]
        : range.startContainer;
      ...
      try {
        ...
        if (
          util.isWysiwygDiv(parentNode) &&
          (oNode.nodeType === 3 || util.isBreak(oNode))
        ) {
        ...
        parentNode.insertBefore(
          oNode,
          parentNode === afterNode ? parentNode.lastChild : afterNode
        );
      } catch (e) {
        ...
        if (!util.isComponent(oNode)) {
          ...
          if (oNode && oNode.nodeType === 3) {
            ...
            const nextText =
              !next || next.nodeType === 1 || util.onlyZeroWidthSpace(next)
                ? ""
                : next.textContent;
    ...
    removeNode: function () {
      ...
      for (let i = startIndex; i <= endIndex; i++) {
        ...
        if (
          item.length === 0 ||
          (item.nodeType === 3 && item.data === undefined)
        ) {
          remove(item);
          continue;
        }
      ...
      for (
        ...
        if (util.isList(originParent)) {
          ...
          if (
            ...
            if (nextList)
              edge.removeArray[edge.removeArray.length - 1].appendChild(
                nextList
              );
            listParent = null;
            nested = false;
    ...
    detachRangeFormatElement: function (
      ...
      // detach loop
      for (
        let i = 0, len = children.length, insNode, lineIndex, next;
        i < len;
        i++
      ) {
        insNode = children[i];
        ...
        if (selectedFormats && lineIndex === -1) {
          ...
          if (!newList && util.isListCell(insNode)) {
            if (
              ...
              (util.isListCell(parent) ||
                util.getArrayItem(insNode.children, util.isList, false))
            ) {
      ...
      if (
        rangeElement.children.length === 0 ||
        rangeElement.textContent.length === 0
      ) {
    ...
    detachList: function (selectedFormats, remove) {
      ...
      for (
        let i = 0, len = selectedFormats.length, r, o, lastIndex, isList;
        i < len;
        i++
      ) {
        lastIndex = i === len - 1;
        ...
        if (
          ...
          if (focusNode) {
            if (!focusNode.nextSibling) {
              ...
              afterNode = util.isBreak(focusNode)
                ? focusNode
                : focusNode.nextSibling;
            }
          }
    ...
    setCommonListStyle: function (el, child) {
      ...
      const children = util.getArrayItem(
        ...
        function (current) {
          return (
            !util.isBreak(current) &&
            !util.onlyZeroWidthSpace(current.textContent.trim())
          );
        },
        true
      );
    ...
    util_getMaintainedNode: function (isRemove, isSizeNode, element) {
      if (!element || isRemove) return null;
      return (
    ...
    util_isMaintainedNode: function (isRemove, isSizeNode, element) {
      if (!element || isRemove || element.nodeType !== 1) return false;
    ...
    nodeChange_oneLine: function (
      ...
      // endContainer reset
      const endConReset =
        isRemoveFormat || endContainer.textContent.length === 0;
      ...
      const endPath = util.getNodePath(
        endContainer,
        pNode,
        !mergeEndCon && !endConReset ? newEndOffset : null
      );
    ...
    nodeChange_startLine: function (
      ...
      (function recursionFunc(current, ancestor) {
        ...
        for (
          let i = 0, len = childNodes.length, vNode, cloneChild;
          i < len;
          i++
        ) {
          ...
          if (!passNode && child === container) {
            ...
            const textNode = util.createTextNode(
              container.nodeType === 1
                ? ""
                : container.substringData(offset, container.length - offset)
            );
    ...
    nodeChange_middleLine: function (
      ...
      if (noneChange || (isRemoveNode && !isRemoveFormat && !removeCheck.v)) {
        return { ancestor: element, endContainer: endContainer };
      }
      ...
      switch (command) {
        ...
        case "save":
          if (typeof options.callBackSave === "function") {
            ...
            this._variable.isChanged &&
            typeof functions.save === "function"
          ) {
            functions.save();
          } else {
            throw Error(
        ...
        default:
          ...
          const cmd =
            nodesMap.indexOf(command) > -1 ? null : util.createElement(command);
      ...
      if (options.fullPage) {
        ...
        context.element.wysiwyg.innerHTML =
          code_html.length > 0
            ...
            : "<" + options.defaultTag + "><br></" + options.defaultTag + ">";
      }
    },
    ...
    makeLine: function (node, requireFormat) {
      ...
      // element
      if (node.nodeType === 1) {
        if (util.disallowedTags(node)) return "";
        if (
          !requireFormat ||
          ...
          return (
            "<" + defaultTag + ">" + node.outerHTML + "</" + defaultTag + ">"
          );
        }
      }
      // text
      if (node.nodeType === 3) {
        if (!requireFormat) return util.HTMLConvertor(node.textContent);
    ...
    cleanTags: function (rowLevelCheck, m, t) {
      ...
      else v = m.match(this.attributesWhitelistRegExp);
    ...
    cleanHTML: function (html, whitelist) {
      ...
      if (this.managedTagsInfo && this.managedTagsInfo.query) {
        ...
        for (
          let i = 0, len = textCompList.length, initMethod, classList;
          i < len;
          i++
        ) {
        ...
        for (
          ...
          if (!node.outerHTML) {
            ...
            returnHTML +=
              ...
              tagIndent +
              node.outerHTML.match(wRegExp("<" + tag + "[^>]*>", "i"))[0] +
              br;
    ...
    init: function (reload, initHTML) {
      ...
      if (!options.iframe && typeof _window.ShadowRoot === "function") {
        ...
        if (this.shadowRoot) this.shadowRootControllerEventTarget = [];
      }
      ...
      for (let key in plugins) {
        ...
        if (
          typeof plugin.checkFileInfo === "function" &&
        ...
        function () {
          this.codeViewDisabledButtons =
            context.element.buttonTray.querySelectorAll(
              '.ke-menu-list button[data-display]:not([class~="ke-code-view-enabled"])'
            );
          this.resizingDisabledButtons =
            context.element.buttonTray.querySelectorAll(
              '.ke-menu-list button[data-display]:not([class~="ke-resizing-enabled"]):not([data-display="MORE"])'
            );
        }.bind(this)
      );
  ...
  const event = {
      ...
      switch (keyStr) {
        ...
        case "S":
          if (shift && options.shortcutsDisable.indexOf("strike") === -1) {
            ...
            !shift &&
            options.shortcutsDisable.indexOf("save") === -1
          ) {
            command = "save";
          }
          break;
    ...
    onMouseDown_wysiwyg: function (e) {
      ...
      // user event
      if (
        typeof functions.onMouseDown === "function" &&
        functions.onMouseDown(e, core) === false
      )
        return;
    ...
    isUneditableNode: function (range, isFront) {
      ...
      if (isElement) {
        ...
        return (
          core.isEdgePoint(container, offset, isFront ? "front" : "end") &&
          siblingNode &&
          siblingNode.nodeType === 1 &&
          siblingNode.getAttribute("contenteditable") === "false"
        );
    ...
    onKeyDown_wysiwyg: function (e) {
      ...
      switch (keyCode) {
        case 8 /** backspace key */:
          ...
          if (
            ...
            if (
              range.collapsed
                ? selectionNode.textContent.length === 1
            ...
            if (
              ...
              if (range.startContainer !== range.endContainer) {
                ...
                if (util.isListCell(prev)) {
                  ...
                  const offset =
                    con.nodeType === 3 ? con.textContent.length : 1;
          ...
          if (
            ...
            const ignoreZWS =
              (commonCon.nodeType === 3 || util.isBreak(commonCon)) &&
        ...
        case 46 /** delete key */:
          ...
          if (
            (util.isFormatElement(selectionNode) ||
              ...
              (util.onlyZeroWidthSpace(selectionNode.nextSibling) &&
                selectionNode.nextSibling.nextSibling === null)) &&
          ...
          if (
            ...
            if (
              next &&
              (util.isList(next) ||
                util.getArrayItem(next.children, util.isList, false))
            ) {
              e.preventDefault();
        ...
        case 13 /** enter key */:
          ...
          if (core.charTypeHTML) {
            ...
            if ((!shift && freeFormatEl) || shift) {
              ...
              enterHTML =
                "<" + formatEl.nodeName + "><br></" + formatEl.nodeName + ">";
            }
          ...
          if (!shift) {
            ...
            if (
              (formatInners && /^H[1-6]$/i.test(formatEl.nodeName)) ||
              /^HR$/i.test(formatEl.nodeName)
            ) {
              e.preventDefault();
              let temp = null;
              const newFormat = core.appendFormatTag(
                formatEl,
                options.defaultTag
              );
              ...
              temp = !temp
                ? newFormat.firstChild
                : temp.appendChild(newFormat.firstChild);
            ...
            if (freeFormatEl) {
              ...
              if (
                !util.isClosureFreeFormatElement(freeFormatEl) &&
                !!children &&
                ((selectionFormat &&
                  range.collapsed &&
                  ...
                  (!children[offset + 1] ||
                    ((!children[offset + 2] ||
                      util.onlyZeroWidthSpace(
                        children[offset + 2].textContent
                      )) &&
                      children[offset + 1].nodeType === 3 &&
                      util.onlyZeroWidthSpace(
                        children[offset + 1].textContent
                      ))) &&
                  offset > 0 &&
                  util.isBreak(children[offset - 1])) ||
                  (!selectionFormat &&
                    util.onlyZeroWidthSpace(selectionNode.textContent) &&
                    util.isBreak(prev) &&
                    (util.isBreak(prev.previousSibling) ||
                      !util.onlyZeroWidthSpace(
                        prev.previousSibling.textContent
                      )) &&
                    (!next ||
                      (!util.isBreak(next) &&
                ...
                const newEl = core.appendFormatTag(
                  ...
                  util.isFormatElement(freeFormatEl.nextElementSibling)
                    ? freeFormatEl.nextElementSibling
                    : null
                );
              ...
              if (selectionFormat) {
                ...
                if (freeFormatEl === focusNode) {
                  focusNode =
                    focusNode.childNodes[
                      wOffset - offset > 1 ? wOffset - 1 : wOffset
                    ];
                }
                ...
                if (
                  !util.isBreak(focusNext) &&
                  !util.isBreak(brPrev) &&
                  (!brNext || util.onlyZeroWidthSpace(brNext))
                ) {
          ...
          if (
            ...
            if (
              (range.commonAncestorContainer.nodeType === 3
                ...
                : true) &&
              util.onlyZeroWidthSpace(formatEl.innerText.trim())
            ) {
              e.preventDefault();
              let newEl = null;
              ...
              if (util.isListCell(rangeEl.parentNode)) {
                ...
                newEl = util.splitElement(
                  formatEl,
                  null,
                  util.getElementDepth(formatEl) - 2
                );
                if (!newEl) {
          ...
          if (
            rangeEl &&
            util.getParentElement(rangeEl, "FIGCAPTION") &&
            util.getParentElement(rangeEl, util.isList)
          ) {
            e.preventDefault();
          ...
          if (fileComponentName) {
            ...
            if (util.isListCell(container.parentNode)) {
              ...
              newEl = util.createElement(
                util.isFormatElement(sibling)
                  ? sibling.nodeName
                  : options.defaultTag
              );
              newEl.innerHTML = "<br>";
      ...
      if (
        ...
        range.collapsed &&
        range.startContainer === range.endContainer &&
        util.isBreak(range.commonAncestorContainer)
      ) {
        const zeroWidth = util.createTextNode(util.zeroWidthSpace);
        core.insertNode(zeroWidth, null, false);
        core.setRange(zeroWidth, 1, zeroWidth, 1);
    ...
    onKeyUp_wysiwyg: function (e) {
      ...
      // user event
      if (
        typeof functions.onKeyUp === "function" &&
        functions.onKeyUp(e, core) === false
      )
        return;
      ...
      if (
        ...
        const oFormatTag = util.createElement(
          util.isFormatElement(core._variable.currentNodes[0])
            ? core._variable.currentNodes[0]
            : options.defaultTag
        );
      ...
      const rangeEl = util.getRangeFormatElement(selectionNode, null);
      if (
        ((!formatEl && range.collapsed) || formatEl === rangeEl) &&
        !util.isComponent(selectionNode) &&
    ...
    onBlur_wysiwyg: function (e) {
      if (core.antiBlur || core._variable.isCodeView) return;
      core.hasFocus = false;
      core.controllersOff(e);
    ...
    resize_editor: function (e) {
      ...
      context.element.wysiwygFrame.style.height =
        context.element.code.style.height =
          (resizeInterval < core._variable.minResizingSize
    ...
    onResize_window: function (e) {
      ...
      if (core._variable.isFullScreen) {
        ...
        context.element.editorArea.style.height =
          core._variable.innerHeight_fullScreen + "px";
        return;
      }
    ...
    hardDelete: function () {
      ...
      if (
        ((sIsCell &&
          !sCell.previousElementSibling &&
          !sCell.parentElement.previousElementSibling) ||
          (eIsCell &&
            !eCell.nextElementSibling &&
            !eCell.parentElement.nextElementSibling)) &&
        sCell !== eCell
      ) {
        if (!sIsCell) {
    ...
    onCopy_wysiwyg: function (e) {
      ...
      // user event
      if (
        typeof functions.onCopy === "function" &&
        !functions.onCopy(e, clipboardData, core)
      ) {
        e.preventDefault();
        e.stopPropagation();
        return false;
    ...
    onCut_wysiwyg: function (e) {
      ...
      // user event
      if (
        typeof functions.onCut === "function" &&
        !functions.onCut(e, clipboardData, core)
      ) {
        e.preventDefault();
        e.stopPropagation();
        return false;
    ...
    onMouseMove_wysiwyg: function (e) {
      ...
      if (component && !core.currentControllerName) {
        ...
        const y =
          e.pageY +
          scrollTop +
          (options.iframe && !options.toolbarContainer
            ? ctxEl.toolbar.offsetHeight
            : 0);
        ...
        if (
          (isList
            ...
            : !util.isFormatElement(component.previousElementSibling)) &&
          y < c + 20
        ) {
          top = componentTop;
          dir = "t";
          ...
          (isList
            ...
            : !util.isFormatElement(component.nextElementSibling)) &&
          y > c + component.offsetHeight - 20
        ) {
    ...
    onLineBreak: function (e) {
      ...
      const format = util.createElement(
        isList
          ? "BR"
          : util.isCell(component.parentNode)
          ? "DIV"
          : options.defaultTag
      );
      ...
      component.parentNode.insertBefore(
        format,
    ...
    addEvent: function () {
      ...
      context.element.toolbar.addEventListener(
        "click",
        event.onClick_toolbar,
        false
      );
      /** editor area */
      eventWysiwyg.addEventListener(
        "mousedown",
        event.onMouseDown_wysiwyg,
        false
      );
      eventWysiwyg.addEventListener("click", event.onClick_wysiwyg, false);
      eventWysiwyg.addEventListener(
        util.isIE ? "textinput" : "input",
        event.onInput_wysiwyg,
        false
      );
      ...
      core.lineBreakerButton.addEventListener(
        "click",
        event.lineBreakerBind.a,
        false
      );
      context.element.lineBreaker_t.addEventListener(
        "mousedown",
        event.lineBreakerBind.t,
        false
      );
      context.element.lineBreaker_b.addEventListener(
        "mousedown",
        event.lineBreakerBind.b,
        false
      );
      ...
      if (context.element.resizingBar) {
        if (/\d+/.test(options.height)) {
          context.element.resizingBar.addEventListener(
            "mousedown",
            event.onMouseDown_resizingBar,
            false
          );
        } else {
    ...
    removeEvent: function () {
      ...
      eventWysiwyg.removeEventListener(
        util.isIE ? "textinput" : "input",
        event.onInput_wysiwyg
      );
      ...
      context.element.lineBreaker_t.removeEventListener(
        "mousedown",
        event.lineBreakerBind.t
      );
      context.element.lineBreaker_b.removeEventListener(
        "mousedown",
        event.lineBreakerBind.b
      );
      ...
      if (context.element.resizingBar) {
        context.element.resizingBar.removeEventListener(
          "mousedown",
          event.onMouseDown_resizingBar
        );
      }
  ...
  const functions = {
     ...
     */
    setToolbarButtons: function (toolbarItem) {
      core.submenuOff();
      core.containerOff();
